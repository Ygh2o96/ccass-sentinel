...
def push_daily_summary(date, collected, errors, total_stocks, total_snaps, highlights):
    """Push daily collection summary (Cat E-style — FYI, no response needed)."""
    lines = [
...
def push_alerts(date, alerts):
    """Push anomaly alerts (Cat F — operator should KNOW). One message per alert."""
    if not alerts:
        return
    
    # Group by severity, send CRITICAL first
...
def push_discovery(date, new_codes):
    """Push new listing discovery (Cat F — operator should KNOW)."""
    if not new_codes:
        return
    msg = (
